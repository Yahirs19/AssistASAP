...
export default function Home() {
  ...
  const GetProductos = async () => {
    const res = await axios.get('/api/prodd')
      .catch((error) => {
  ...
  const GetProveedores = async () => {
    const res = await axios.get('/api/provv')
      .catch((error) => {
        console.log('catch: ', error.message);
      });
  ...
  const DeleteProductos = async (userId: string) => {
    const resp = await axios.delete("/api/prodd", {
      params: { id: userId }
    }).catch((error) => {
      console.log("catch: ", error.message);
    });
...
? prods.filter((dato) => dato.name.toLocaleLowerCase().includes(search.toLocaleLowerCase()))
: prods;
...
const handleProveedorChange = (e: ChangeEvent<HTMLSelectElement>) => {
  ...
  const validateForm = () => {
    ...
    if (
        ...
        pro.price === null || pro.price === undefined || pro.price.toString().trim() === '' || // Convertir 'price' a string y aplicar 'trim()'
  ...
  return (
    ...
    {showForm && (
        <div style={{
            ...
            <form style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}  onSubmit={handleSubmit}>
                ...
                <label htmlFor="name" style={{ marginBottom: '10px', color: '#333', fontSize: '14px', fontWeight: 'bold', width: '100%', textAlign: 'left' }}>Nombre:</label>
                <input onChange={HandleChange} value={pro.name} type="text" name="name" style={{ marginBottom: '15px', padding: '10px', width: '100%', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />

                <label htmlFor="imageUrl" style={{ marginBottom: '10px', color: '#333', fontSize: '14px', fontWeight: 'bold', width: '100%', textAlign: 'left' }}>Url de la imagen:</label>
                <input onChange={HandleChange} value={pro.imageUrl} type="text" name="imageUrl" style={{ marginBottom: '15px', padding: '10px', width: '100%', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />

                <label htmlFor="price" style={{ marginBottom: '10px', color: '#333', fontSize: '14px', fontWeight: 'bold', width: '100%', textAlign: 'left' }}>Precio:</label>
                <input onChange={HandleChange} value={pro.price} type="text" name="price" style={{ marginBottom: '15px', padding: '10px', width: '100%', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />

                <label htmlFor="cantidad" style={{ marginBottom: '10px', color: '#333', fontSize: '14px', fontWeight: 'bold', width: '100%', textAlign: 'left' }}>Cantidad:</label>
                <input onChange={HandleChange} value={pro.cantidad} type="text" name="cantidad" style={{ marginBottom: '15px', padding: '10px', width: '100%', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />

                <label htmlFor="slug" style={{ marginBottom: '10px', color: '#333', fontSize: '14px', fontWeight: 'bold', width: '100%', textAlign: 'left' }}>Modelo:</label>
                <input onChange={HandleChange} value={pro.slug} type="text" name="slug" style={{ marginBottom: '15px', padding: '10px', width: '100%', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />
            ...
            <label htmlFor="Proveedor" style={{ marginBottom: '10px', color: '#333', fontSize: '14px', fontWeight: 'bold', width: '100%', textAlign: 'left' }}>Proveedor:</label>
                ...
                <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <button type="submit" style={{ padding: '8px 16px', backgroundColor: '#4CAF50', color: '#FFFFFF', border: 'none', borderRadius: '4px', cursor: 'pointer', marginRight: '8px', fontSize: '14px' }}>Añadir</button>
                    <button onClick={() => {setShowForm(false), ResetProductos(), setError('')}} style={{ padding: '8px 16px', backgroundColor: '#F87171', color: '#FFFFFF', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' }}>Cancelar</button>
                </div>
            </form>
        </div>
    )}
            ...
            {isDialogOpen && (
                <div style={{
                  ...
                  zIndex: 1000
                    ...
                    <p style={{ color: '#721c24', textAlign: 'center'}}>El producto ya existe. Por favor, ingrese otro producto.</p>
                    <button onClick={handleDialogClose} style={{ backgroundColor: '#721c24', color: '#fff', padding: '8px 16px', borderRadius: '4px', border: 'none' }}>
                        Cerrar
                    </button>
                </div>
    ...
    {showForm2 && (
        <div style={{
            ...
            <h1 style={{ fontWeight: 700, textAlign: 'center' }}>ACTUALIZAR PRODUCTO</h1>
            <form style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }} onSubmit={handleSubmitUpdate}>
            {error && <p style={{ color: 'red' }}>{error}</p>}
                <label htmlFor="name" style={{ marginBottom: '10px', color: '#333', fontSize: '14px', fontWeight: 'bold', width: '100%', textAlign: 'left' }}>Nombre:</label>
                <input onChange={HandleChange} value={pro.name} readOnly type="text" name="name" style={{ marginBottom: '15px', padding: '10px', width: '100%', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />
                ...
                <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <button type="submit" style={{ padding: '8px 16px', backgroundColor: '#4CAF50', color: '#FFFFFF', border: 'none', borderRadius: '4px', cursor: 'pointer', marginRight: '8px', fontSize: '14px' }}>Actualizar</button>
                    <button onClick={() => {setShowForm2(false), ResetProductos(), setError('')}} style={{ padding: '8px 16px', backgroundColor: '#F87171', color: '#FFFFFF', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' }}>Cancelar</button>
                </div>
            </form>
        </div>
    )}
...
{isDialogOpen2 && (
                <div style={{
                  ...
                  zIndex: 1000
                    ...
                    <h3 style={{ color: '#721c24', textAlign: 'center'  }}>Advertencia</h3>
                    <p style={{ color: '#721c24', textAlign: 'center'  }}>Ingresa un numero valido</p>
                    <button onClick={handleDialogClose2} style={{ backgroundColor: '#721c24', color: '#fff', padding: '8px 16px', borderRadius: '4px', border: 'none' }}>
                        Cerrar
                    </button>
                </div>
...
<div style={{
        ...
        {isDialogOpen5 && (
                <div style={{
                  ...
                  zIndex: 1000
                    ...
                    <p style={{ color: '#721c24', textAlign: 'center'  }}>Este producto no existe</p>
                    <button onClick={handleDialogClose5} style={{ backgroundColor: '#721c24', color: '#fff', padding: '8px 16px', borderRadius: '4px', border: 'none' }}>
                        Cerrar
                    </button>
                </div>
        ...
        <br></br>
                    ...
                    <button onClick={() => setShowForm(true)} style={{ margin: '20px', padding: '10px 20px', backgroundColor: '#4CAF50', color: '#FFFFFF', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' }}>Añadir Producto</button>
                    <br></br>

<table style={{ marginTop: '8%',borderCollapse: 'collapse',marginLeft: '0px', width: '100%', maxWidth: '1700px',margin: 'auto', backgroundColor: '#FFF5', borderRadius: '20px', boxShadow: '0 8px 16px -4px rgba(0, 0, 0, 0.1), 0 0 0 1px rgba(0, 0, 0, 0.05)', backdropFilter: 'blur(7px)', border: 'none', overflow: 'hidden' }}>
  
  <thead>
    <tr style={{ borderBottom: '1px solid #E5E7EB', borderRadius: '20px 20px 0 0' }}>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>ID</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Nombre</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Imagen</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Precio</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Cantidad</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Modelo</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Descripción</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Categoria</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>usuarioAdminID</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>proveedorID</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Fecha de registro</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>Fecha de actualizacion</th>
      <th style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center', borderRadius: '0 0 20px 20px' }}>Acciones</th>
    </tr>
  </thead>
  <tbody>
  {resultados.map((prodss: Product) => (
  <tr key={prodss.id} style={{ borderBottom: '1px solid #E5E7EB' }}>
    <td style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>{prodss.id}</td>
    <td style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>{prodss.name}</td>
    <td style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>
      <img src={prodss.imageUrl}  style={{ maxWidth: '60px', maxHeight: '60px' }} />
    </td>
    <td style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>${prodss.price}</td>
    ...
    <td style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>{prodss.UpdatedDate.toString()}</td>
    <td style={{ border: '1px solid #E5E7EB', padding: '8px', textAlign: 'center' }}>
      <button onClick={() => {setShowForm2(true), EditProductos(prodss.id) }} style={{ backgroundColor: '#4CAF50', color: '#FFFFFF', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer' }}>Editar</button>
      <button onClick={() => handleDeleteClick(prodss.id)} style={{ backgroundColor: '#F87171', color: '#FFFFFF', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', marginTop:'6px' }}>Eliminar</button>
    </td>
  </tr>
))}
...
{isDialogOpen3 && (
    <div style={{
        ...
        <h3 style={{ color: '#721c24', textAlign: 'center' }}>Confirmación de eliminación</h3>;
        <p style={{ color: '#721c24', textAlign: 'center' }}>¿Estás seguro de que deseas eliminar este producto?</p>
        <br></br>
        <div style={{
            display: 'flex',
